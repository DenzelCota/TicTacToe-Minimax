...
int minimax(std::vector<std::vector<char>>& tablero, bool esJugadorMax) {
    ...
    if(esJugadorMax) {
        ...
        for(int i = 0; i < 3; ++i) {
            for(int j = 0; j < 3; ++j) {
                if(tablero[i][j] == VACIO) {
                    tablero[i][j] = IA;
                    mejorValor = std::max(mejorValor, minimax(tablero, !esJugadorMax));
                    tablero[i][j] = VACIO;
                }
            }
        }
        ...
        for(int i = 0; i < 3; ++i) {
            for(int j = 0; j < 3; ++j) {
                if(tablero[i][j] == VACIO) {
                    tablero[i][j] = JUGADOR;
                    mejorValor = std::min(mejorValor, minimax(tablero, !esJugadorMax));
                    tablero[i][j] = VACIO;
                }
            }
        }
...
for(int i = 0; i < 3; ++i) {
        for(int j = 0; j < 3; ++j) {
            if(tablero[i][j] == VACIO) {
                tablero[i][j] = IA;
                int valor = minimax(tablero, false);
                tablero[i][j] = VACIO;
